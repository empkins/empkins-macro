...
class StrideDetection:
    ...
    def spatial_features(self, sampling_rate: float = 60, turn_thres: float = 100):
        ...
        df_left = self._convert_position_and_orientation(
            self._min_vel_event_list["left"],
            self._get_gait_sequence(self._min_vel_event_list["left"]),
        )
        df_right = self._convert_position_and_orientation(
            self._min_vel_event_list["right"],
            self._get_gait_sequence(self._min_vel_event_list["right"]),
        )
        ...
        spatial_paras_r = spatial_paras_r.calculate(
            ...
            positions=df_right["RightFoot"]["pos"],
            orientations=df_right["RightFoot"]["ori"],
            sampling_rate_hz=sampling_rate,
        )
    ...
    def _stride_detection(self):
        ...
        stride_events = {
            "left": self._get_stride_events(self.data["FootContacts"]["left"]),
            "right": self._get_stride_events(self.data["FootContacts"]["right"]),
        }

        stride_events = {
            "left": self._clean_stride_events(
                self.data["LeftFoot"], stride_events["left"]
            ),
            "right": self._clean_stride_events(
                self.data["RightFoot"], stride_events["right"]
            ),
        }

        stride_event_times = {
            "left": self._get_stride_event_times(stride_events["left"]),
            "right": self._get_stride_event_times(stride_events["right"]),
        }

        min_vel = {
            "left": self._get_min_vel(
                stride_event_times["left"], self.data["LeftFoot"]
            ),
            "right": self._get_min_vel(
                stride_event_times["right"], self.data["RightFoot"]
            ),
        }

        min_vel_event_list = {
            "left": self._build_min_vel_event_list(
                stride_event_times["left"], min_vel["left"]
            ).dropna(),
            "right": self._build_min_vel_event_list(
    ...
    def _convert_position_and_orientation(
        ...
        for start, end in zip(sequence_list["start"], sequence_list["end"]):
            cleaned_data = cleaned_data.append(self.data[start:end])
    ...
    def _get_stride_events(self, foot_contacts: pd.DataFrame) -> pd.DataFrame:
    ...
    def _clean_stride_events(
        self, segment_data: pd.DataFrame, stride_events: pd.DataFrame, thres: float = 50
    ) -> pd.DataFrame:
    ...
    def _find_matching_stride_events(self, stride_events: pd.DataFrame) -> pd.DataFrame:
        ...
        stride_events["ic"] = pd.DataFrame(
            [
                ...
                for tc, tc_shifted in zip(
                    stride_events["tc"], stride_events["tc_shifted"]
                )
            ]
        )
    ...
    def _get_stride_event_times(
        self, stride_events_cleaned: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        stride_event_times = self._find_matching_stride_events(stride_event_times)

        return stride_event_times

    def _get_min_vel(
        self, stride_events_cleaned: pd.DataFrame, segment_data: pd.DataFrame
    ) -> pd.DataFrame:
        min_vel_list = []
    ...
    def _build_min_vel_event_list(
        self, stride_events_cleaned: pd.DataFrame, min_vel_events: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _get_gait_sequence(self, min_vel_events: pd.DataFrame) -> pd.DataFrame:
        ...
        sequ["start"] = sequ["start"].shift(
            1, fill_value=min_vel_events.iloc[0]["start"]
        )
