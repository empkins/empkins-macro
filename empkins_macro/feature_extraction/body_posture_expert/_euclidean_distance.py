...
def euclidean_distance(data: pd.DataFrame, body_part: Sequence[str], **kwargs) -> pd.DataFrame:
    ...
    start_end = _euclidean_distance_threshold(distance, **kwargs)
    ...
    out = {("_".join(body_part), "euclidean_distance", channel, axis): out}
...
def _euclidean_distance_threshold(data: pd.DataFrame, **kwargs) -> pd.DataFrame:
    min_distance_cutoff = kwargs.get("minimal_distance_cutoff", 120)
    distance_thres = kwargs.get("distance_threshold", 20)
